...
# View experiments
To directly view an experiment, select an experiment from the drop down menu, and click the "View Experiment" button. The experiment will open in a new window. [see online version for this content]
...
df <- data.frame(exp = c("XT E1", "XT E2", "SPSS E1"),
                 ...
                 n = c(22, 36, 19))
kable(df , col.names = c("Exp.",	"one-exmplar means",
                         "three-exemplar means",	"one-exemplar sd",
                         "three-exmplar sd", "sample size")) %>%
...
Using a random effect meta-analytic model and these value, we obtain a meta-analytic effect size estimate. We estimate this effect size to be `r round(model$b, 2)`.
...
kable(es_1_calc, digits = 2, 
      col.names = c("condition", "mean", "variance", "sample size"))  %>%
  kable_styling(font_size = 12) 
...
In the Main Text, we report our analyses collapsed across all three stimulus categories (animals, vehicles and vegetables). Here we present the effect sizes for each experiment separately for the different stimulus categories. While there is some variability in effect size by category (the effect is generally larger for animals), this variability is small relative to the effect of condition order. [see online version for this content]
...
LF_effect_sizes_cat <- LF_means_cat %>%
    ...
    rename(high = u.d,
         low = l.d,
         d_var = var.d)  %>%
   mutate(exp_recoded = LF_means_cat$exp_recoded,
