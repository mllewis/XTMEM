...
## what's going on with the other measures
## A note on ES calculation (http://jakewestfall.org/blog/index.php/category/effect-size/)
##  Effect size by category 
The plot below shows the effect size for each experiment (facet) for each of the three categories in our stimuli (animals, vegetables and vehicles). While there is some variability in effect size by category (the effect is larger for animals), this variability is small relative to the effect of condition order. For reference, Table 1 describes the characteristics of each of the experiments.
