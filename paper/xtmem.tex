...
Xu and Tenenbaum (2007a) present evidence that learners are able to
infer subordinate word meanings by attending to statistical properties
of the input: It would be a \enquote{suspicious coincidence} to observe
three subordinate exemplars with the same label if the label refered to
the basic level, and thus, the label must have a subordinate meaning.
Perone, Smith, and Samuelson (2011) replicate this effect, but argue
that it only occurs when the training exemplars are presented
simultaneously---an ecologically unlikely circumstance. Across 12
studies, we attempt to resolve this empirical discrepancy by
systematically manipulating parameters of the experimental design that
differed between the two sets of studies. We replicate the data patterns
of both sets of authors. However, our exploration of the parameter space
leads us to a different conclusion than SPSS: We find that the
suspicious coincidence effect is robust to the presentation timing of
exemplars, but is sensitive to order effects. Specifically, we only
observe the suspicious coincidence effect when the single exemplar
...
occurred first. Why might the ordering matter? While our data are not
able to directly speak to this question, we speculate that this
difference may be due to the fact that learners are tracking exemplar
frequency across trials. When the single exemplar trial occurs second,
it is effectively the \emph{fourth} exemplar from the same subordinate
category that learners have observed in a row; not a single exemplar.
This hypothesis predicts that participants should be less likely to
...
evidence consistent with this prediction in an analysis of
generalizations to the basic level across all experiments with the 3-1
ordering (\emph{t}(581) = -2.23; \emph{p} = 0.03; \emph{d} = -0.18
{[}-0.2, -0.16{]}).
...
sampling effects more broadly; it is an artifact of the experimental
setup. This finding does, however, highlight the influence of seemingly
minor experimental design parameters on the observed pattern of data. In
the present set of experiments, experiments with the 1-3 versus 3-1
ordering differed by an effect size of 1.45 --- a sizeable difference
that is likely to invite an unwarrented theoretical explanation.

Experimental design parameters are especially important to consider in
the context of replication. When conducting a replication of an existing
finding, small design parameters may influence the magnitude of the
effect (Lewis \& Frank, 2016) and even its presence (Phillips et al.,
2015). To identify the cause of an effects, experimenters must verify
that an effect is robust to theoretically-irrelevant design decisions,
and replicators must reproduce the original design decisions with as
...
to make efficient inferences on the basis of limited data.
