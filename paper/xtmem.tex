...
meaning (dalmation) and the level meaning (dog). Yet both children and
adults successfully learn noun meanings at the correct level of
abstraction from similar evidence. Xu and Tenenbaum (2007a) provided an
explanation to this apparent puzzle: Learners assume that examples are
...
three-exemplar trials are presented first, participants remember these
exemplars when they subsequently encounter the one-exemplar trial, and
consequently do not interpret the single exemplar as the \emph{only}
observed exemplar from the target category. In sum, although we
replicate SPSS exactly, our full set of studies leads us to a different
interpretation of the data. We conclude that the \enquote{suspicious
...
in which the learner first observes a trial with three subordinate
peppers followed by a second trial with only a single pepper. Although
the two trials were intended to be interpretted as independent from each
other, their co-occurrence in the task may have suggested to
participants that they were pragmatically related, leading participants
...
with our findings. It also makes a second prediction: If learners
interpret the single pepper in the second trial as the fourth pepper
exemplar, then they should be even less likely to generalize the basic
level under these conditions compared to a case when three peppers are
presented in the first trial. We find some evidence consistent with this
...
Our data also suggest that the 3-1 ordering interacts with trial order:
In experiments with the sequential 3-1 ordering (Exp. 10-12), we see a
reliable reversal of the suspicious coincident effect (\emph{t}(289) =
-2.61; \emph{p} = 0.01; \emph{d} = -0.3 {[}-0.32, -0.28{]}). This
reversal is attributable to increased generalizations to the basic level
in the three-subordinate, sequential condition, compared to the
three-subordinate, simultaneous condition. Though speculative, we
...
Both XT's and SPSS's work address an important puzzle in the
psychological sciences: How do learners learn concepts at multiple
levels of abstraction? Their work focuses on a simplified version of
this puzzle where the learner must determine the corresponding labels to
known concepts. Our findings support the idea that learners solve this
puzzle by making probabilistic inferences about the level of abstraction
that is most likely given the observed data (the original
\enquote{suspicious coincidence} effect). Importantly, an account
relying on probabilistic inferences is consistent both with the original
XT set of findings and the observed trial order effects. Our data add to
the growing body of work suggesting that this effect may arise as a
consequence of pragmatic reasoning in language comprehension (Frank \&
Goodman, 2014; Goodman \& Frank, 2016; Shafto et al., 2012). Such
