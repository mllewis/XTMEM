...
```{r stim_fig, out.width = "90%", fig.align = "center", fig.cap = "Sample learning and generalization sets. On a given trial, participants saw one or three exemplars of the same level from the learning set, followed by all exemplars from the generalization set (along with the generalization sets from the other categories)."}
...
```{r violin_plot_response_to_reviewers, include = FALSE}
ms_recoded <- ms %>%
  left_join(exp_key) %>%
  ungroup()  %>%
