...
exp_table <- exp_key %>%
  ...
  select(exp_recoded, n, timing, order, blocking, one_3sub_label, d_string) %>%
  mutate(exp_recoded = as.numeric(exp_recoded)) %>%
  arrange(exp_recoded)

kable(exp_table, align = c('c', 'r', 'r', 'r', "r", "r", "r"), 
      #col.names = c("Experiment \nNum.", "N", "Timing", "Condition \nOrder",
      #              "Trial \nOrder", "Label", "Effect Size"),
      col.names = c("Experiment", "N", "Timing", "Order",
                     "Blocking", "Label", "Effect Size"),
...
ggplot(all_es) +
  ...
  geom_pointrange(aes(x = jitter(source, 1.4),  y = d, ymax = high, 
                      ymin = low, color = one_3sub_label, shape = fct_rev(blocking)), size = .5) +
...
mod_df <- data.frame(fixed_effect = c("Intercept", 
                                        ...
                                        "Same vs. different label", 
                                        "Random vs. blocked trial order"),
...
kable(mod_df, align = c('l', 'r', 'r', 'r'), digits = 2,
