// XTMEM 11 -- same-random-3-sequential
// Overview: 
//      (1) Helper
...
var train_three_sub_pics = shuffle([["c1_sub3", "c1_sub4", "c1_sub5"], 
                                    ["c2_sub3", "c2_sub4", "c2_sub5"], 
                                    ["c3_sub3", "c3_sub4", "c3_sub5"]])

var train_other_pics = shuffle([["c1_sub3"],
                                ["c2_sub3"],
                                ["c3_sub3"],
...
var  blocks = ["three_sub", "other"]

// PRE-LOAD IMAGES
var all_pics = [].concat.apply([], [train_three_sub_pics, train_other_pics], "mrfrog.png")
...
var experiment = {
    ...
    test: function() {  
        ...
        // is this a one trial?
        var one_trial = current_train_pics.length == 1
        
        if (one_trial) { // this is randomly a 1 trials - find matching label from three sub
          
          // gets name of item for same category on the 3sub trials
          objs = current_train_pics[0]
          var current_cat_type = objs.split("_")[0] 
          if (train_three_sub_pics[0][0].split("_")[0] == current_cat_type){
            index = 0
          } else if (train_three_sub_pics[1][0].split("_")[0] == current_cat_type){
            index = 1
          } else if (train_three_sub_pics[2][0].split("_")[0] == current_cat_type){
            index = 2
          }
          current_word = sub_words[index]
        } else if (blocks[0] == "three_sub"){
    ...
    next: function() {
      if (selected.length > 0){
          ...
          } else if(current_train_pics.length == 1){
              current_cond = "one"
          }  
